...
def build_model_plan(selection: dict[str, Any], prior: str = "NONE") -> dict[str, Any]:
    task_models = []
    for task_type, item in selection.get("tasks", {}).items():
        ...
        task_models.append({"task_id":task_type,"candidates":candidates,"selection":{"model_id":item["selected"]["model_id"],"reason":item["reason"],"alternatives_rejected":[c["model_id"] for c in candidates[1:]]}})
