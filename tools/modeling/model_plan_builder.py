...
def build_model_plan(selection: dict[str, Any], prior: str = "NONE") -> dict[str, Any]:
    ...
    return {"artifact_type":"ModelPlan","schema_version":"0.9","status":"VALIDATED","task_models":task_models,"selection_principles":["确定性七维评分优先于LLM主观猜测","先硬约束淘汰，再比较候选模型","保留基线与至少一个可比较备选","D/E仅作为先验，不作为固定工作流"],"d_or_e_prior":prior}

def build_model_spec(task: dict[str, Any], problem_map: dict[str, Any], proposal: dict[str, Any] | None = None) -> dict[str, Any]:
    model_id=task["selection"]["model_id"]; model=BY_ID[model_id]; task_id=task["task_id"]; proposal=proposal or {}
    binding=proposal.get("data_binding") if isinstance(proposal.get("data_binding"),dict) else None
    objective=f"针对任务 {task_id}，使用{model.name}建立可验证模型并服务于题目目标。"
    spec={"artifact_type":"ModelSpec","schema_version":"0.9","status":"DRAFT","model_id":model_id,"task_id":task_id,"name":model.name,"objective":objective,"variables":[{"symbol":"X","role":"input","meaning":"由题目或数据确定的输入变量"},{"symbol":"Y","role":"target","meaning":"任务对应的目标量"}],"parameters":[],"equations":[f"{model.name}的模型方程由后续计算阶段根据真实数据/机理确定"],"constraints":[],"assumptions":list(model.assumptions),"solution_method":model.tool,"validation_plan":["留出或交叉验证（适用时）","与基线模型比较","残差/误差或约束满足性检查","敏感性或稳健性检查（适用时）"],"limitations":list(model.limitations)}
    if binding: spec["data_binding"]=binding
    return spec
