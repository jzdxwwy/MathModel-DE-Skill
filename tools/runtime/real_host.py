"""Real LLM host adapter through V0.9-E verified numerical execution."""
...
from .verification_engine import verify_executions
...
def _proposal_for_task(output: object, task_id: str) -> dict:
    ...
    if isinstance(items, list):
        for item in items:
            if isinstance(item, dict) and item.get("task_id") == task_id: return item
    if output.get("task_id") == task_id: return output
    return {}


class RealLLMHostAdapter:
    """Run ingestion, semantic front-end, deterministic selection, binding, compute and verification."""
    ...
    def run(self, request: HostRequest) -> HostResponse:
        ...
        runtime.run(ctx,[RuntimeStage("00-start",request.instruction+"\nReturn ONLY JSON ProblemSpec. Required: artifact_type, schema_version, status, problem_id, source{title,mode}, tasks[]. Never invent missing data.",s00),RuntimeStage("01-analysis","Return ONLY JSON ProblemMap. Map every ProblemSpec task exactly once with identical task_id. Required task_id, objective, inputs, outputs.",s01),RuntimeStage("02-data","Return ONLY JSON semantic DataProfile additions. Deterministic ingestion facts are authoritative; do not rewrite them.",s02)])
        problem_map=json.loads((project_dir/"artifacts/problem-map.json").read_text(encoding="utf-8")); data_profile=json.loads((project_dir/"artifacts/data-profile.json").read_text(encoding="utf-8")); selection=select_models(problem_map,data_profile); ctx.metadata["model_selection"]=selection; proposal_output={}
        def s03(s):
            nonlocal proposal_output
            save_output("03-modeling")(s); proposal_output=s.metadata["model_outputs"]["03-modeling"].get("output",{}); plan=build_model_plan(selection); spec_dir=s.project_dir/"artifacts/model-spec"; spec_dir.mkdir(parents=True,exist_ok=True)
            for item in plan["task_models"]:
                proposal=_proposal_for_task(proposal_output,item["task_id"]); task=next((t for t in problem_map.get("tasks",[]) if t.get("task_id")==item["task_id"]),{}); binding=resolve_binding(item["selection"]["model_id"],task,data_profile,proposal); item["data_binding"]=binding; resolved=dict(proposal); resolved["data_binding"]=binding; spec=build_model_spec(item,problem_map,resolved); e=validate_artifact(self.repo_root,spec,"ModelSpec")
                ...
                candidates=[{"model_id":c["model_id"],"name":c["name"],"method":c["model_id"],"evidence":[c["fit_rationale"],f"评分={c.get('score',0)}"],"metrics":{"selection_score":c.get('score',0)},"rank":rank} for rank,c in enumerate(item["candidates"],1)]
                ...
                if e: raise RuntimeError("ModelComparison gate failed: "+"; ".join(e))
            ...
            plan["status"]="VALIDATED"; pp=persist_artifact(s.project_dir,"model-plan",plan); s.register_artifact("ModelPlan",pp)
        def s04(s):
            ...
            p=persist_artifact(s.project_dir,"tool-dispatch",dispatch); s.register_artifact("ToolDispatchPlan",p); registry=__import__(".tool_registry",package=__package__,fromlist=["ToolRegistry"]).ToolRegistry(); register_default_tools(registry); executions=ToolExecutionEngine(self.repo_root,registry).execute(dispatch,project_dir,request.problem_input.get("problem", "")); ep=s.project_dir/"artifacts/v09-execution.json"; ep.write_text(json.dumps(executions,ensure_ascii=False,indent=2),encoding="utf-8"); s.register_artifact("V09Execution",ep)
            reports=verify_executions(project_dir,executions,dispatch); vp=s.project_dir/"artifacts/verification-summary.json"; s.register_artifact("VerificationSummary",vp); s.metadata["verification_reports"]=reports
            if any(r.get("gate_decision")=="FAIL" for r in reports): raise RuntimeError("06-verification gate failed: at least one execution failed verification")
        runtime.run(ctx,[RuntimeStage("03-modeling","Use metadata.model_selection as authoritative. Return JSON task_models with optional semantic binding hints only. Never override selected models.",s03),RuntimeStage("04-compute","Execute closed dispatch. BLOCKED bindings remain INPUT_BLOCKED; never fabricate data.",s04)])
        manifest=ctx.persist(); decisions=[r.get("gate_decision") for r in ctx.metadata.get("verification_reports",[])]; overall="PASS" if decisions and all(d=="PASS" for d in decisions) else "PASS_WITH_WARNINGS" if decisions and all(d in {"PASS","PASS_WITH_WARNINGS"} for d in decisions) else "FAIL"
        return HostResponse(status="completed" if overall!="FAIL" else "failed",task_id=task_id,manifest=str(manifest),message=f"V0.9-E completed: compute + deterministic verification; overall={overall}.",artifacts={k:str(v) for k,v in ctx.artifacts.items()})
