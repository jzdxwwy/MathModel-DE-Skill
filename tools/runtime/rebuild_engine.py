"""V1.0-D controlled automatic clean-rebuild engine.

Executes only a registered ToolRegistry tool. It never launches arbitrary
shell commands. Source hashes are checked before execution and the rebuild
uses a fresh run directory so the frozen result cannot be mutated.
"""
...
def verify_input_hashes(contract: dict[str, Any], project_dir: Path) -> list[str]:
    ...
    for item in contract.get("inputs", []):
        rel = Path(str(item.get("path", ""))); path = project_dir / rel
...
class CleanRebuildEngine:
    ...
    def __init__(self, repo_root: Path, registry: ToolRegistry):
        self.repo_root, self.registry = Path(repo_root), registry

    def rebuild(self, contract: dict[str, Any], project_dir: Path) -> dict[str, Any]:
        project_dir = Path(project_dir)
        errors = validate_contract(contract, self.registry)
        input_failures = verify_input_hashes(contract, project_dir)
        run_id = f"rebuild-{uuid.uuid4().hex[:12]}"
        run_dir = project_dir / "runs" / run_id; run_dir.mkdir(parents=True, exist_ok=True)
        manifest: dict[str, Any] = {
            "artifact_type": "RebuildRunManifest", "schema_version": "1.0-D", "run_id": run_id,
            "reference_run_id": contract.get("run_id"), "model_id": contract.get("model_id"),
            "tool": contract.get("tool"), "parameters": contract.get("parameters", {}),
            "python": sys.version.split()[0], "status": "BLOCKED", "checks": []}
        ...
        payload = {
            "task_id": f"rebuild:{contract['run_id']}", "model_id": contract["model_id"],
            "project_dir": str(project_dir), "run_dir": str(run_dir), "repo_root": str(self.repo_root),
            "inputs": [x["path"] for x in contract.get("inputs", [])],
            "parameters": contract.get("parameters", {}),
            "binding": {"binding_status": "VERIFIED_REBUILD_CONTRACT"}, "rebuild": True}
        try:
            result = self.registry.invoke(contract["tool"], **payload)
            result_payload = result if isinstance(result, dict) else {"result": result}
            bundle = {
                "artifact_type": "ResultBundle", "schema_version": "0.9", "status": "VALIDATED",
                "run_id": run_id, "model_id": contract["model_id"],
                "outputs": result_payload.get("outputs", []), "metrics": result_payload.get("metrics", {}),
                "artifacts": result_payload.get("artifacts", []), "warnings": [],
                "provenance": {"input_refs": [x["path"] for x in contract.get("inputs", [])],
                               "code_ref": contract["tool"], "environment": sys.version.split()[0],
                               "rebuild_of": contract["run_id"]}}
            _write_json(run_dir / "result-bundle.json", bundle)
            manifest["status"] = "REBUILD_COMPLETE"
            manifest["checks"].append({"check_id": "D03_REGISTERED_TOOL", "decision": PASS})
            manifest["outputs"] = [str(run_dir / "result-bundle.json")]
        except Exception as exc:
            manifest["status"] = "FAILED"
            manifest["checks"].append({"check_id": "D03_REGISTERED_TOOL", "decision": FAIL,
                                        "refs": [f"{type(exc).__name__}: {exc}"]})
        _write_json(run_dir / "rebuild-run-manifest.json", manifest)
        return manifest

def build_rebuild_contract(reference_run_id: str, model_id: str, tool: str, project_dir: Path,
                           input_paths: list[str], parameters: dict[str, Any], *,
    ...
    return {
        "artifact_type": "RebuildContract", "schema_version": "1.0-D", "run_id": reference_run_id,
        "model_id": model_id, "tool": tool, "inputs": inputs, "parameters": parameters,
        "environment": environment or {"python": sys.version.split()[0]},
        "expected_result": {"model_id": model_id, "schema_version": "0.9"},
