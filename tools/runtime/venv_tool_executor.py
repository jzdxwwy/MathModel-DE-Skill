...
class VenvToolExecutor:
    def __init__(self,*,tool_resolver:Callable[[str],str]):
        self.tool_resolver=tool_resolver

    def execute(self,contract:dict[str,Any],*,project_dir:str|Path,output_dir:str|Path)->dict[str,Any]:
        if contract.get("artifact_type")!="VenvToolExecutionContract" or contract.get("schema_version")!="1.0-K":
            raise ValueError("invalid V1.0-K contract")
        if contract.get("policy")!={"allow_network":False,"allow_shell":False,"isolation_required":True}:
            raise PermissionError("V1.0-K policy violation")
        ...
        for item in contract.get("input_hashes",[]):
            p=root/item["path"]
            if not p.is_file() or _sha256(p)!=item["sha256"]:
                raise ValueError(f"input hash mismatch: {item['path']}")
        module=self.tool_resolver(contract["tool"])
        if not module or module.startswith("-") or any(x in module for x in [" ",";","|","&"]):
            raise ValueError("invalid host-resolved module")
