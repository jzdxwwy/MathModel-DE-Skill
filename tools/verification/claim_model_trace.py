...
def _find(root:Path,names:list[str]):
    for name in names:
        for p in (root/name,root/"modeling"/name,root/"reference"/name,root/"reference"/"modeling"/name,root/"paper"/name):
            if p.is_file(): return p
    return None

def _hash_expr(s:str)->str:
    # Conservative normalization: whitespace only. No algebraic equivalence inference.
...
def evaluate_claim_model_trace(root:str|Path,*,atol=1e-8,rtol=1e-6):
    ...
    if not all((pe_path,idx_path,spec_path)):
        missing=[n for n,p in (("paper-evidence.json",pe_path),("claim-evidence-index.json",idx_path),("model-spec.json",spec_path)) if p is None]
        return {"artifact_type":"ClaimModelTrace","schema_version":"1.0-S","status":"DRAFT","run_id":root.name,"claims":[],"gate_decision":NOT_RUN,"violations":["missing: "+", ".join(missing)]}
    pe,idx,spec=_load(pe_path),_load(idx_path),_load(spec_path)
    if not all((pe,idx,spec)):
        return {"artifact_type":"ClaimModelTrace","schema_version":"1.0-S","status":"DRAFT","run_id":root.name,"claims":[],"gate_decision":FAIL,"violations":["invalid JSON"]}
    ib={str(c.get("claim_id")):c for c in idx.get("claims",[]) or []}
    violations=[]; claims=[]
    for pc in pe.get("claims",[]) or []:
        cid=str(pc.get("claim_id","")); ic=ib.get(cid)
        if not cid: violations.append("claim missing claim_id"); continue
        if ic is None: violations.append(f"{cid}: missing ClaimEvidenceIndex"); continue
        model_id=str(pc.get("model_id") or ic.get("model_id") or spec.get("model_id") or "")
        if model_id and model_id!=str(spec.get("model_id","")):
        ...
        for i,o in enumerate(eq_obs):
            ...
            eqb.append({"claim_ref":cref,"model_ref":mref,"normalized_match":match,"claim_expression_hash":_hash_expr(cref) if cref else "","model_expression_hash":_hash_expr(meq) if meq else ""})
            if not match: violations.append(f"{cid}: equation mismatch or unresolved {mref}")
        for o in par_obs:
            sym=str(o.get("symbol","")); cv=o.get("value",o.get("claim_value"))
            params=[p for p in spec.get("parameters",[]) or [] if str(p.get("symbol"))==sym]
            if len(params)!=1:
                pb.append({"symbol":sym,"claim_value":cv,"model_value":None,"comparison":"MISSING","atol":atol,"rtol":rtol}); violations.append(f"{cid}: parameter {sym} not uniquely defined")
                continue
        ...
        closed=has_obs and all(x["normalized_match"] for x in eqb) and all(x["comparison"]=="MATCH" for x in pb) and (not model_id or model_id==str(spec.get("model_id")))
        claims.append({"claim_id":cid,"model_id":model_id,"equation_bindings":eqb,"parameter_bindings":pb,"trace_status":"CLOSED" if closed else "OPEN"})
    decision=FAIL if violations or any(c["trace_status"]!="CLOSED" for c in claims) else (PASS if claims else NOT_RUN)
    out={"artifact_type":"ClaimModelTrace","schema_version":"1.0-S","status":"VALIDATED" if decision==PASS else "DRAFT","run_id":root.name,"claims":claims,"gate_decision":decision,"violations":violations}
